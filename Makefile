...
SOURCES			:=	minishell.c																														\
					...
					prompt.c																														\
																																					\
					parser/parser.c																													\
					parser/split/cmds.c				parser/split/args.c																				\
					parser/cmd.c					parser/prev.c					parser/loop.c					parser/error.c					\
					parser/quotes.c					parser/flags.c					parser/special.c												\
																																					\
					env/init.c						env/dict.c						env/env.c						env/parse.c						\
																																					\
					builtins/echo.c					builtins/cd.c					builtins/pwd.c					builtins/export.c				\
					builtins/unset.c				builtins/env.c					builtins/exit.c													\
																																					\
					process/exec.c					process/builtin.c				process/extern.c				process/file.c					\
																																					\
					signal.c																														\
																																					\
					error.c							debug.c
...
ifeq ($(strip $(CLIBRARY)), 1)
	@$(MAKE) -s _compile_lib SOURCES='$(strip $(SOURCES))' CFLAGS='$(strip $(CFLAGS))' SOURCES_DIR='$(strip $(SOURCES_DIR))'
else
	@$(MAKE) -s _compile_exec SOURCES='$(strip $(SOURCES))' CFLAGS='$(strip $(CFLAGS))' SOURCES_DIR='$(strip $(SOURCES_DIR))'
endif
	@$(call print_done)
...
_compile_smart:	_progress_file _set_debug _hook_name
	@$(if $(shell [[ (-e $(NAME)) && ($(FILE_TOTAL) == 0) ]] && echo 1),																			\
		$(call print_nothing),																														\
		$(MAKE) -s _compile CFLAGS='$(strip $(CFLAGS))' SOURCES='$(strip $(SOURCES))' SOURCES_DIR='$(strip $(SOURCES_DIR))';						\
	)
...
_progress_info:
	...
	@if [[ ($(UI_COMPILE) == 0) && ($(UI_MESSAGE) == 1) ]]; then \
